<template>
    <div>
        ...
        <a-table :dataSource="dataSource" :columns="columns" :pagination="false" :loading="loading" style="margin-top: 20px">
            <template #bodyCell="{column , record}">
                <template v-if="column.dataIndex === 'seatType'">
                    <template v-for="item in seat_type" :key="item.code">
        ...
        <a-modal v-model:open="open"
                 title="enter dailyTrainCarriage info"
                 :closable="false"
                 :mask="true"
                 :maskClosable="false"
                 :footer="[]"
        >
            ...
            >
                <a-form-item
                        ...
                        name="dailyTrainId"
                >
                    <a-input v-model:value="dailyTrainCarriage.dailyTrainId" disabled/>
                </a-form-item>
                <a-form-item
                        label="火车箱号"
                        name="trainIndex"
                >
                    <a-input v-model:value="dailyTrainCarriage.trainIndex"/>
                </a-form-item>
                <a-form-item
                        label="座位类型"
                        name="seatType"
                >
                    <a-select v-model:value="dailyTrainCarriage.seatType">
                        <a-select-option v-for="item in seat_type" :value="item.code" :key="item.code">{{item.description}}</a-select-option>
                    </a-select>
                </a-form-item>
                <a-form-item
                        label="排数"
                ...
                <a-form-item>
                    <a-popconfirm placement="topLeft" ok-text="Yes" cancel-text="No" @confirm="confirm">
                        <template #title>
                            are you sure to add this dailyTrainCarriage?
                        </template>
...
const confirm = async () => {
    let res = await doPost('/business/dailyTrainCarriage/admin/save', dailyTrainCarriage);
    if (res) {
        resetTrainCarriage()
...
function resetTrainCarriage() {
    Object.assign(dailyTrainCarriage,{
        dailyTrainId:  routeParams.value.dailyTrainId,
        id:  ``,
        trainIndex:  ``,
        seatType:  `1`,
...
const queryTrainCarriageList = () => {
    loading.value = true
    axios.get("/business/dailyTrainCarriage/admin/query-list?currentPage=" + pagination.current + "&pageSize=" + pagination.pageSize+"&dailyTrainId="+routeParams.value.dailyTrainId)
        .then(res => {
            loading.value = false
            if (res) {
                if (res.data.success) {
...
const queryTrainCarriageSeatType = ()=>{
    loading.value = true
    axios.get('/business/get-enum/business.enums.Seat')
        .then(res =>{
            loading.value = false
            if (res) {
                seat_type.value = res.data
            }
        })
}

const handleEdit = (record) => {
    Object.assign(dailyTrainCarriage,record)//该方法相当于BeanUtil.copyProperties
    open.value = true
}

const handleDelete = (record)=>{
    axios.delete('/business/dailyTrainCarriage/admin/delete/'+record.id)
        .then(resp => {
            if (resp) {
